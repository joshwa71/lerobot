...
#
# lerobot/lerobot/common/policies/fcdp/fcdp_dataset.py
...
def fcdp_collate_fn(batch: List[Tuple[Dict[str, Any], torch.Tensor, torch.Tensor]], config: FCDPConfig) -> Tuple[Dict[str, Any], torch.Tensor, torch.Tensor]:
    """
    Custom collate function for FCDPDataset.
    Handles 'history_context' and 'fail_traj_context_optional' which can contain lists of (dict or None).
    Also handles target_action_trajectory and target_padding_mask.
    """
    policy_input_dicts, target_action_trajectories, target_padding_masks = zip(*batch)
    ...
    if history_len > 0:
        ...
        if template_item_hist is None:
            ...
            template_item_hist_fallback = {
                ...
                'action': torch.zeros((config.policy_action_dim,), dtype=torch.float32),
                'observation_visual': {
    ...
    # Handle fail_traj_context_optional
    ...
    collated_target_action_trajectories = default_collate(target_action_trajectories)
    collated_target_padding_masks = default_collate(target_padding_masks)

    return collated_policy_input_dict, collated_target_action_trajectories, collated_target_padding_masks


class FCDPDataset(Dataset):
    def __init__(
        self,
        config: FCDPConfig,
        success_dataset_repo_id: str,
        mixed_dataset_repo_id: Optional[str] = None,
        ...
        if mixed_dataset_repo_id:
            ...
            logger.info("No mixed_dataset_repo_id provided. FCDPDataset will run in 'success-only' mode.")
        ...
        if not self.episode_map:
            raise ValueError("FCDPDataset init resulted in no valid samples. Check dataset contents and pairing.")
    ...
    def _build_episode_map(self):
        # Filter out samples where we cannot construct a full horizon
        for ep_idx in range(self.success_ds_meta.total_episodes):
            ...
            for t in range(0, ep_len, self.config.frame_skip_rate):
                # Check if we can get a full horizon of actions
                if t + self.config.horizon * self.config.frame_skip_rate <= ep_len:
                    ...
                    if ep_idx + 1 < self.mixed_ds_meta.total_episodes:
                        ...
                        if next_ep_meta.get('success', False):
                            ...
                            for t_succ in range(0, succ_ep_len, self.config.frame_skip_rate):
                                # Check if we can get a full horizon of actions
                                if t_succ + self.config.horizon * self.config.frame_skip_rate <= succ_ep_len:
                                    self.episode_map.append({
                                        "ds_idx": 1,
    ...
    def _load_trajectory_timestep_data(self, ds_meta: LeRobotDatasetMetadata, hf_ds: datasets.Dataset, 
        ...
        data_out['action_with_done'] = torch.cat([raw_action, done_signal])
    ...
    def _get_history_and_current(self, ds_meta, hf_ds, ep_data_idx, ep_idx_in_ds, t_in_ep) -> Tuple[List[Dict[str, Any] | None], Dict[str, Any]]:
        ...
        for k_hist_offset in range(self.config.history_len, 0, -1):
            ...
            if hist_t_original >= 0:
                ...
                history_data_list.append({
                    ...
                    "action": step_data['action_with_done']
                })
            else:
        ...
        return history_data_list, current_s_o

    def _get_action_trajectory(self, ds_meta, hf_ds, ep_data_idx, ep_idx_in_ds, t_in_ep) -> Tuple[torch.Tensor, torch.Tensor]:
        """Load horizon future actions starting from current timestep."""
        ep_len = ds_meta.episodes[ep_idx_in_ds]['length']
        action_trajectory = []
        padding_mask = []
        
        for h in range(self.config.horizon):
            future_t = t_in_ep + h * self.config.frame_skip_rate
            if future_t < ep_len:
                step_data = self._load_trajectory_timestep_data(ds_meta, hf_ds, ep_data_idx, ep_idx_in_ds, future_t)
                action_trajectory.append(step_data['action_with_done'])
                padding_mask.append(1.0)  # Valid action
            else:
                # Padding with zeros
                action_trajectory.append(torch.zeros(self.config.policy_action_dim, dtype=torch.float32))
                padding_mask.append(0.0)  # Padded action
        
        action_trajectory = torch.stack(action_trajectory, dim=0)  # (horizon, action_dim + 1)
        padding_mask = torch.tensor(padding_mask, dtype=torch.float32)  # (horizon,)
        
        return action_trajectory, padding_mask
    ...
    def __getitem__(self, idx):
        ...
        if mode == "standard":
            ...
            history_context, current_state_obs = self._get_history_and_current(ds_meta, hf_ds, ep_data_idx, ep_idx_in_ds, t_in_ep)
            target_action_trajectory, target_padding_mask = self._get_action_trajectory(ds_meta, hf_ds, ep_data_idx, ep_idx_in_ds, t_in_ep)
            ...
            history_context, current_state_obs = self._get_history_and_current(ds_meta, hf_ds, ep_data_idx, succ_ep_idx, t_in_succ_ep)
            target_action_trajectory, target_padding_mask = self._get_action_trajectory(ds_meta, hf_ds, ep_data_idx, succ_ep_idx, t_in_succ_ep)
        ...
        return policy_input_dict, target_action_trajectory, target_padding_mask