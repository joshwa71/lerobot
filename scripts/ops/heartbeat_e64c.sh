...
RUNNER=/home/josh/lerobot/scripts/vla_analysis/run_e64c_r64_after_r128.sh
UNIT=e64c-r64
...
R=/home/josh/lerobot
u=$(systemctl is-active e64c-r64 2>/dev/null); true
sp=$(ls -d $R/outputs/train/loraft_baseline_r64/task*/checkpoints/005000 2>/dev/null | wc -l)
sd=$(ls $R/outputs/analysis/e60/seeds_spec_r64_e*.json 2>/dev/null | wc -l)
step=$(grep -oE "step:[0-9]+K?" $R/outputs/e64c_r64.log 2>/dev/null | tail -1 | cut -d: -f2)
dk=$(df --output=pcent /home/josh | tail -1 | tr -dc '0-9')
er=$(grep -cE "Traceback|OutOfMemoryError|CUDA out of memory|\[FAIL\]" $R/outputs/e64c_r64.log 2>/dev/null)
fin=$(grep -c "E64c r64 ladder COMPLETE" $R/outputs/e64c_r64.log 2>/dev/null)
gpu=$(nvidia-smi --query-gpu=memory.used --format=csv,noheader 2>/dev/null | tr -d ' ')
echo "unit=$u spec64=$sp/10 rows64=$sd/10 step=${step:-0} disk=${dk}% err=$er fin=$fin gpu=${gpu:-NA}"
REMOTE
}

ship_state() {
  local alive pass fail
  # match the repo script OR the immutable run-copy it is launched from (24 Aug)
  pgrep -f "ship_e64(spec|_batch_to_cold|_fixed)\.sh" >/dev/null && alive=yes || alive=NO
  pass=$(grep -c "^PASS " "$STATUS" 2>/dev/null); true
  fail=$(grep -cE "^FAIL" "$STATUS" 2>/dev/null); true
  pass=${pass:-0}; fail=${fail:-0}
  echo "ship=$alive pass=$pass/2 fail=$fail"
}
...
emit "heartbeat-C armed: r64 ladder + specialist cold ship; poll ${POLL}s, forced beat every $((POLL*HEARTBEAT_EVERY/3600))h"
while true; do
  s_ship=$(ship_state)
  if state=$(remote_state 2>/dev/null) && [ -n "$state" ]; then
    ...
    rows=$(sed -E 's/.* rows64=([0-9]+)\/10 .*/\1/' <<<"$state")
    ...
    prev_fail=${nfail:-0}
    [[ "$s_ship" == *"ship=NO"* ]] && [[ "$s_ship" != *"pass=2/2"* ]] && emit "SHIPPER PROCESS GONE with transfers incomplete | $s_ship"

    if [ "$unit" != "active" ] && [ "$unit" != "activating" ]; then
      if [ "${fin:-0}" -ge 1 ] 2>/dev/null || [ "${rows:-0}" -ge 10 ] 2>/dev/null; then
        emit "R64 LADDER COMPLETE | $state | $s_ship"
        [[ "$s_ship" == *"pass=2/2"* ]] && { emit "SHIP ALSO COMPLETE — exiting"; exit 0; }
      else
