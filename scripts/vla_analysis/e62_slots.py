...
PAIRS = {"merged6x2": [("gemma_expert", 4, 6), ("gemma_expert", 8, 10),
                       ("language_model", 5, 7), ("language_model", 9, 11),
                       ("language_model", 13, 15)]}
OUT_DIR = "/home/josh/lerobot/outputs/analysis/e62"
...
with open(f"{OUT_DIR}/slots_e62.out", "w") as fh:
    for name, rd in RUNS.items():
        ...
        for t in range(4):
            ...
            for m in mods:
                ...
                ev = sum(cnt for lt in range(t + 1, 5) for s, cnt in upds[lt][m].items() if s in c50)
                cells.append(f"{short(m)}:{ev}")
            print(f" victim t{t}/e{ENV[t]}: " + "  ".join(cells), file=fh)
print("wrote", OUT_DIR + "/slots_e62.out")
