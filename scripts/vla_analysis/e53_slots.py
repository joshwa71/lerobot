#!/usr/bin/env python3
"""E53 slot autopsy: corefrac + spread-A vs the E52 comparators (foldin, plain).
Per module x task: read mass, update events, core50, effnum, self-coverage; per
victim<-writer pair: bleed (RTO) + events into victim core50; cumulative RTO; and
the logged-IoU validation (mean pairwise weighted read Jaccard over all modules,
raw access counts — the slots.py formula validated against wandb memory_iou).
Output: outputs/analysis/e53/slots_e53.{out,json}"""
import json, math, os
...
OUT_DIR = "/home/josh/lerobot/outputs/analysis/e53"
...
def wjacc(a, b):
    """weighted jaccard on raw access counts (sum min / sum max over union)"""
    keys = set(a) | set(b)
    inter = union = 0.0
    for k in keys:
        x = a.get(k, 0.0); y = b.get(k, 0.0)
...
def analyze(name, run_dir, fh):
    ...
    print(f"\n-- victim<-writer bleed (victim read-mass %% on writer-updated slots | writer events into victim core50) --", file=fh)
    res["pairs"] = {}
    for v in range(5):
    ...
    print(f"\n-- cumulative RTO (read mass on ANY later writer's updates; /coreX = core50-restricted) --", file=fh)
    for v in range(4):
        cells = []
        for m in mods:
            r = reads[v][m]
    ...
    # logged-IoU validation: mean pairwise weighted Jaccard over all modules
    jac = []
    per_pair = {}
    for i in range(5):
        for j in range(i + 1, 5):
            v = sum(wjacc(reads[i][m], reads[j][m]) for m in mods) / len(mods)
            per_pair[f"t{i}-t{j}"] = round(v, 4)
            jac.append(v)
    res["read_iou_mean"] = round(sum(jac) / len(jac), 4)
    res["read_iou_pairs"] = per_pair
    print(f"\n-- computed read IoU (mean pairwise weighted Jaccard, all modules): {res['read_iou_mean']:.4f}", file=fh)
    print(f"   pairs: {per_pair}", file=fh)
    return res


all_res = {}
with open(os.path.join(OUT_DIR, "slots_e53.out"), "w") as fh:
    for name, d in RUNS.items():
        all_res[name] = analyze(name, d, fh)
        print(f"[{name}] read_iou={all_res[name]['read_iou_mean']}", flush=True)
json.dump(all_res, open(os.path.join(OUT_DIR, "slots_e53.json"), "w"))
print("done; wrote", OUT_DIR + "/slots_e53.{out,json}")
