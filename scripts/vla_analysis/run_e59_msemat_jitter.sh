#!/bin/bash
# E59 landing battery, GPU proc A (per Josh, 5 Aug 26): MSE forgetting matrix
...
set -eo pipefail
SP=/home/josh/lerobot/outputs/analysis/e59
mkdir -p $SP
SCRIPTS=/home/josh/lerobot/scripts/vla_analysis
ROOT=/home/josh/lerobot
BASE=$ROOT/outputs/train
SEQ=libero_10_seq5_jw_interleave_e681012_v791113_prepass_beta4corefrac_topt3072_lr2x_steps5k
...
RD=$BASE/$SEQ
export MSEMAT_RUN_DIR=$RD MSEMAT_STEPS=005000,010000,015000,020000,025000 MSEMAT_OUT=$SP/mse_matrix_interleave.jsonl
python $SCRIPTS/mse_matrix2.py \
  --policy.path="$RD/checkpoints/005000/pretrained_model" \
  "${COMMON_ARGS[@]}" \
  --output_dir=$SP/msemat_out_interleave --job_name=msemat_interleave
echo "=== MSE matrix done ==="

export PROBE_RUN_DIR=$RD PROBE_CKPTS="t0:025000,t3:025000,t4:025000" PROBE_OUT=$SP/probe_jitter_interleave.jsonl PROBE_SWAP_SLOTS=1
python $SCRIPTS/probe_jitter.py \
  --policy.path="$RD/checkpoints/025000/pretrained_model" \
  "${COMMON_ARGS[@]}" \
  --output_dir=$SP/jitter_out_interleave --job_name=jitter_interleave
echo "=== E59 probe battery COMPLETE ==="
