#!/bin/bash
# E60 landing battery: MSE matrix + jitter (run_e60_msemat_jitter.sh), slot
# autopsy (e60_slots.py, bigsearch vs interleave vs B triple), harvest-bank
# rescore TAG=bigsearch (e7 58 vs spec 60 — the radius read at near-parity).
set -o pipefail
ROOT=/home/josh/lerobot
LOG=$ROOT/outputs/e60_battery.log
exec >> "$LOG" 2>&1
echo "=== E60 battery started $(date -u) ==="
...
bash scripts/vla_analysis/run_e60_msemat_jitter.sh || echo "[battery] msemat/jitter FAILED"
python scripts/vla_analysis/e60_slots.py || echo "[battery] autopsy FAILED"

SP=$ROOT/outputs/analysis/e56_offtrail
RD=$ROOT/outputs/train/libero_10_seq5_jw_bigsearch_e4to16_v5to13_prepass_beta4corefrac_topt3072_lr2x_steps5k
...
echo "=== E60 BATTERY COMPLETE $(date -u) ==="
