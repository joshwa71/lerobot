#!/usr/bin/env python3
"""Feature-separability probe (research_log Entry 35 addendum discussion).
...
def main(cfg: SequentialOnlineConfig):
    ...
    expert_layers = unwrapped.model.paligemma_with_expert.gemma_expert.model.layers
    ...
    def make_hook(L):
        def hook(module, args, kwargs=None):
            x = args[0]
            if x.dim() == 3:
                captured[L].append(x.detach().float().mean(dim=1).cpu())
        return hook
