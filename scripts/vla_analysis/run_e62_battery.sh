#!/bin/bash
# E62 landing battery: msemat + jitter -> slot autopsy (site-bleed on 5 shared
# pairs + prior-core events incl. the solo E14/E16 depth-lever cells) ->
# harvest-bank rescore vs the e7 specialist (SCORE_FEAT_LAYER=4: below this
# model's first VLM bank at 5). Cloned from run_e61_battery.sh.
set -o pipefail
ROOT=/home/josh/lerobot
LOG=$ROOT/outputs/e62_battery.log
exec >> "$LOG" 2>&1
echo "=== E62 battery started $(date -u) ==="
...
bash scripts/vla_analysis/run_e62_msemat_jitter.sh || echo "[battery] msemat/jitter FAILED"
python scripts/vla_analysis/e62_slots.py || echo "[battery] autopsy FAILED"
...
echo "=== E62 BATTERY COMPLETE $(date -u) ==="
