#!/bin/bash
# FULL-FINETUNE BASELINE 2 (Josh, 7 Aug): the LIBERO-90-finetuned pi05 (stage-1 substrate,
# = the SAME base every memory run starts from) -> full finetune on libero_10
# (no memory) 50K -> 4-seed eval. Answers the "same substrate" question: what
# does full-backbone finetuning buy from OUR starting point. Train args mirror
# the E31/stage-1 convention verbatim (bs32, grad-ckpt REQUIRED for
# full-backbone, warmup 4K/decay 50K, pi05 default LR); in-run eval disabled
# (eval_freq > steps) — the 4-seed campaign is the instrument.
# Preemption-safe: save_freq=10000 + auto-resume.
# Gated on the `fullft-l10` unit exiting (runs after baseline 1).
set -o pipefail
...
while true; do
  st=$(systemctl is-active fullft-l10 2>/dev/null)
  [ "$st" = "active" ] || [ "$st" = "activating" ] || break
  sleep 300
done
echo "=== fullft-l90l10: fullft-l10 exited (state=$st) — starting $(date -u) ==="
...
PI05_BASE="/home/josh/lerobot/outputs/train/libero_90_pi05_base_nomem_50k/checkpoints/last/pretrained_model"
RUN=libero_10_pi05_fullft_froml90_nomem_50k
...
export CAMP_TAG=fullft_l90_l10
export CAMP_OUT=$ROOT/outputs/analysis/e60/seeds_fullft_l90_l10.json
if [ ! -f "$CAMP_OUT" ]; then
  python scripts/vla_analysis/eval_seeds_campaign.py \
    ...
    --seed=1000 \
    --output_dir=/tmp/camp_fullft_l90_l10 \
    || echo "[FAIL] fullft_l90_l10 campaign"
fi
echo "=== FULLFT-L90L10 COMPLETE $(date -u) ==="
