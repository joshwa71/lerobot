#!/bin/bash
# == E53 addendum: expert_anchor_weight BISECTION — trying 0.25 ====================
# The expert text-anchor brackets from BOTH sides; neither endpoint threads the gate:
#   w=0.5 (anchor05): expert famIoU 0.11-0.14 OK but core50 330-457 -> OVER-COMPACTED
#                     (capacity fail; E21 separation-by-collapse).
#   w=0.1 (anchor01): core50 3668-9881 huge but famIoU 0.48-0.68 / bg 0.18-0.45 ->
#                     UNDER-SEPARATED (separation fail; the pre-registered flip).
# The per-task-CONSTANT pooled-LM-instruction anchor's separation power and its
# compaction are the SAME routing-breadth knob read two ways (E21), so a viable
# weight — if one exists — is boxed into (0.1, 0.5): a possibly tight or empty window.
# This run bisects at w=0.25 (~geometric mean 0.224; leans capacity-side). The gate
# routes the next step: famIoU still >0.20 -> up toward 0.35; core50 <800 -> down
# toward 0.15; BOTH pass -> the window exists, graduate to A-phase + sequential. Run
# tag anchor01 -> anchor025 so the warm-up re-trains FRESH (skip-guards key on
# ARM_TAG). VLM anchor [1.0,0.5] untouched. If 0.25 also fails, the expert-anchor is
# likely dead and arm 2 (plain spread + corefrac) stands as the spread verdict.
# =================================================================================
...
export ARM_TAG=layermax_A_anchor025_nofilm_e2468_v10121416
...
export EXPERT_ANCHOR_W=0.25   # E53-addendum: bisect — 0.5 over-compacts / 0.1 under-separates
...
export SEQ_RUN=libero_10_seq5_jw_layermax_A_anchor025nofilm_e2468_v10121416_beta4corefrac_topt3072_lr2x_steps5k
