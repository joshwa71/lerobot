#!/bin/bash
# PROBE 9 (10k) — SEP CURVE point sep=5.0 (goal B, far end)
# Single delta from P6 (c0.05/sep2.0/loc0.25/rq512): separation 2.0 -> 5.0.
# Tests where the sep curve turns over: does famIoU keep dropping, or does the
# shrink-to-disjoint shortcut finally kick in (core50/effnum collapse, j/p ratio
# falls = halves re-correlate) once separation dominates the objective?
# At sep=5.0 the weighted separation term (~5 x 0.08 = 0.4) exceeds MSE (~0.21),
# so this is the regime where over-separation risk is highest. Watch capacity hard.
# Same compressed 10k schedule. Gate: famIoU<=~0.28 AND core50>=~1500 (jointly).

set -eo pipefail
echo "Probe 9 (standard c=0.05 + sep 5.0 + rq512) started on $(hostname) at $(date)"
...
RUN=libero_90_pi05_8_10_12_14_probe10k_standard_c0.05_sep5.0_rq512
...
echo "Probe 9 (standard c=0.05 + sep 5.0 + rq512) completed at $(date)"
