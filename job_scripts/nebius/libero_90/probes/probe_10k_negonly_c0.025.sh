#!/bin/bash
# PROBE 1 (10k) — DOSE knob: SupCon negatives_only at HALF weight (0.05 -> 0.025).
#
# Isolates whether the Entry-21 capacity collapse was driven by the WEIGHT
# (fixable by dosing down) or by the negatives_only STRUCTURE (no anti-collapse
# term at any weight). Identical to the failed run except weight 0.05 -> 0.025.
#
# Same compressed 10k schedule as probe-C (steps=10000, scheduler_decay_steps
# left at 40000 so it auto-scales to warmup 1000 / decay 10000) -> directly
# comparable to audit_heldout_probeC_10k. THROWAWAY screen, not resumed.
#
# Read CAPACITY, not just IoU (the Entry-21 trap):
#   GOOD: held-out L14 core50 >= ~1500, query_intra_sim <= ~0.90,
#         routing_intra_task_support_L14 >= ~5000, family IoU <= ~0.28
#   BAD : core50 < ~1000 or query_intra_sim > 0.95 (over-compact, will collapse)
# Prediction: negonly has no anti-collapse force; 0.025 may only DELAY collapse.

set -eo pipefail

echo "Probe 1 (negonly, c=0.025) started on $(hostname) at $(date)"
...
RUN=libero_90_pi05_8_10_12_14_probe10k_negonly_c0.025
...
lerobot-train \
  ...
  --policy.memory_layer.contrastive_loss_weight=0.025 \
...
echo "Probe 1 (negonly, c=0.025) completed at $(date)"
