#!/bin/bash
# PROBE 2 (10k) — STRUCTURE knob: STANDARD SupCon (negatives_only=FALSE) at the
# same weight 0.05.
#
# This is the mechanistically-targeted fix for the Entry-21 collapse. Standard
# SupCon keeps same-task samples in the denominator, restoring the intra-task
# uniformity term that is LOAD-BEARING for per-task capacity, while still
# pushing tasks apart (inter-task negatives remain). Identical to the failed run
# except contrastive_negatives_only true -> false.
#
# Same compressed 10k schedule as probe-C (steps=10000, decay auto-scales to
# 10k) -> directly comparable to audit_heldout_probeC_10k. THROWAWAY screen.
#
# Read CAPACITY, not just IoU:
#   GOOD: held-out L14 core50 >= ~1500, query_intra_sim <= ~0.90,
#         routing_intra_task_support_L14 >= ~5000, family IoU <= ~0.28
#   BAD : family IoU >= ~0.32 (~control -> no separation gained), OR
#         core50 < ~1000 / query_intra_sim > 0.95 (still over-compact)
# Prediction: favored fix. query_intra_sim should settle ~0.85-0.90, capacity
# preserved, separation milder than negonly but still below control.

set -eo pipefail

echo "Probe 2 (standard SupCon, c=0.05) started on $(hostname) at $(date)"
...
RUN=libero_90_pi05_8_10_12_14_probe10k_standard_c0.05
...
echo "Probe 2 (standard SupCon, c=0.05) completed at $(date)"
