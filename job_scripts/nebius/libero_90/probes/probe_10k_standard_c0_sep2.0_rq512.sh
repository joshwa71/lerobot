#!/bin/bash
# PROBE 7 (10k) — IS CONTRASTIVE EVEN NEEDED? (goal A)
# Single delta from P6 (c0.05/sep2.0/loc0.25/rq512): contrastive_loss_weight 0.05 -> 0.
# Hypothesis: with the routing-separation loss doing the heavy lifting (rq512, sep2.0),
# query-space SupCon may be unnecessary. If held-out famIoU/core50 hold ~ P6 without it,
# contrastive is redundant (and we lose its capacity-compaction side effect for free).
# NOTE: contrastive_loss_weight=0 GATES the query-sim diagnostic (memory_lite.py:621),
#   so query_intra/inter_sim will NOT be logged for this run — read capacity from the
#   held-out audit (core50/effnum/subkey), which is the ground truth regardless.
# Same compressed 10k schedule. Gate: famIoU<=~0.28 AND core50>=~1500 (jointly).

set -eo pipefail
echo "Probe 7 (contrastive=0 + sep 2.0 + rq512) started on $(hostname) at $(date)"
...
RUN=libero_90_pi05_8_10_12_14_probe10k_standard_c0_sep2.0_rq512
...
lerobot-train \
  ...
  --policy.memory_layer.contrastive_loss_weight=0 \
...
echo "Probe 7 (contrastive=0 + sep 2.0 + rq512) completed at $(date)"
