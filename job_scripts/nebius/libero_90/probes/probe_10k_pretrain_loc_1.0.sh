#!/bin/bash
# PROBE L (10k): does routing_intra_task_locality_weight=1.0 make the locality
# loss BIND at the late layers (L12/L14)?
#
# Single-knob delta vs the 40k control
# (libero_90_pi05_8_10_12_14_film_lora_2_sample_contrastive_0.01_sep_0.25_loc_0.25_sup_128_2048_knn_36_40k):
#   - routing_intra_task_locality_weight 0.25 -> 1.0
#   - steps 40000 -> 10000 (TRUNCATED FULL RUN: scheduler warmup/decay kept at
#     4000/40000 so a passing probe can be continued to 40k with --resume=true)
#   - save_freq 20000 -> 10000 (checkpoint at 10k for the held-out routing audit)
#   - eval never fires (eval_freq=20000 > steps); probes are judged on routing
#     structure + MSE guardrail, not rollouts.
#
# Pass gates (vs control wandb history):
#   - train/routing_intra_task_support_L14 < ~5.5K by 10k and still descending
#     (control was flat at ~7.9-8.2K through 10k; plateau 7.4K at 40k)
#   - train/mse_loss@10k within ~10% of control's 0.261
#   - held-out audit (libero_10 forward-only): L14 core50/effnum down ~2x
# Failure routing: L14 flat at 1.0 -> next probe is PER-LAYER locality
# (heavy on 12/14), not a larger global weight.

set -eo pipefail

echo "Probe L (locality 1.0) started on $(hostname) at $(date)"
...
RUN=libero_90_pi05_8_10_12_14_probe10k_loc_1.0
...
lerobot-train \
  ...
  --policy.memory_layer.routing_intra_task_locality_weight=1.0 \
...
echo "Probe L completed at $(date)"
