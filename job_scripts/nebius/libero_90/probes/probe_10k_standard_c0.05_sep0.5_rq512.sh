#!/bin/bash
# PROBE 4 (10k) — DIRECT SLOT-SPACE SEPARATION (favored bet).
# Standard SupCon 0.05 (capacity-safe) + routing_inter_task_separation 0.25 -> 0.5.
#
# Question: can we separate held-out family footprints by pushing task slot-
# distributions APART directly (translation), rather than via query-space
# collapse (shrinkage)? routing_inter_task_separation acts on the joint-slot
# distribution retrieval actually uses (E7), penalizing pairwise task overlap;
# it rewards disjointness without rewarding smallness -- the decoupling we want.
# Never swept in the pi05 regime. Identical to P2 except sep 0.25 -> 0.5.
#
...
#   (the win we haven't found: famIoU down WHILE core50 stays up)
# BAD: famIoU ~ control (no separation) OR core50 collapse (sep induced shrinkage
#   anyway -> reserve test is sep 0.5 + locality 0).

set -eo pipefail
echo "Probe 4 (standard SupCon c=0.05 + sep 0.5) started on $(hostname) at $(date)"
...
RUN=libero_90_pi05_8_10_12_14_probe10k_standard_c0.05_sep0.5_rq512
...
lerobot-train \
  ...
  --policy.memory_layer.routing_inter_task_separation_weight=0.5 \
  --policy.memory_layer.routing_query_queue=512

echo "Probe 4 (standard SupCon c=0.05 + sep 0.5) completed at $(date)"
