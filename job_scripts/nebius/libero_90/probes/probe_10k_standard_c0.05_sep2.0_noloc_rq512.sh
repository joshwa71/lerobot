#!/bin/bash
# PROBE 10 (10k) — DOES LOCALITY DO ANYTHING? (goal C)
# Single delta from P6 (c0.05/sep2.0/loc0.25/rq512): locality_weight 0.25 -> 0,
# and the support-band flags dropped (min/max_support only feed the now-off locality
# term -- inert with locality=0, removed here for clarity). Separation stays 2.0.
# Hypothesis (Josh): locality contributes ~nothing (it was 0.6% of MSE in the Entry-24
# magnitude audit and probe-L showed it has near-zero leverage). If held-out
# famIoU/core50 match P6 without it, locality is dead and can be dropped permanently.
# Same compressed 10k schedule. Gate: compare DIRECTLY to P6 (sep2.0 WITH locality):
#   ~same famIoU/core50 -> locality is inert; materially worse capacity -> it was
#   load-bearing as an anti-shrink floor after all.

set -eo pipefail
echo "Probe 10 (standard c=0.05 + sep 2.0 + NO locality + rq512) started on $(hostname) at $(date)"
...
RUN=libero_90_pi05_8_10_12_14_probe10k_standard_c0.05_sep2.0_noloc_rq512
...
echo "Probe 10 (standard c=0.05 + sep 2.0 + NO locality + rq512) completed at $(date)"
