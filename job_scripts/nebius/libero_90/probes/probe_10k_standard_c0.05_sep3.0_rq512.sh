#!/bin/bash
# PROBE 8 (10k) — SEP CURVE point sep=3.0 (goal B)
# Single delta from P6 (c0.05/sep2.0/loc0.25/rq512): separation 2.0 -> 3.0.
# With P6 (sep2.0) at held-out famIoU 0.311 / core50 2368 (decoupled, no shrink),
# this maps whether famIoU keeps falling toward the ~0.28 gate while capacity holds.
# Same compressed 10k schedule. Gate: famIoU<=~0.28 AND core50>=~1500 (jointly);
# watch the shrink-shortcut signatures (effnum/core50 down, j/p ratio down).

set -eo pipefail
echo "Probe 8 (standard c=0.05 + sep 3.0 + rq512) started on $(hostname) at $(date)"
...
RUN=libero_90_pi05_8_10_12_14_probe10k_standard_c0.05_sep3.0_rq512
...
lerobot-train \
  ...
  --policy.memory_layer.routing_inter_task_separation_weight=3.0 \
  --policy.memory_layer.routing_query_queue=512

echo "Probe 8 (standard c=0.05 + sep 3.0 + rq512) completed at $(date)"
