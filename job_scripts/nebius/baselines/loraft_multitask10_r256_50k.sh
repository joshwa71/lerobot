...
# the paper table:
#   r=256 / lora_alpha=64  (alpha/r = 0.25 held, same as the r32@a8 specialists
#                           and the naive-seq r256@a64 foil -> same effective
#                           update scale; 426M trainable, ~120x our per-token
#                           active params and ~1.9x our per-step trained budget)
...
# Eval = the 4-seed campaign (run_e64_lora_r256_queue.sh), not in-run.
#
...
set -eo pipefail
echo "E64 multitask-LoRA-10 r256/50k started on $(hostname) at $(date)"
ROOT_DIR=/home/josh/lerobot
BASE_CKPT="$ROOT_DIR/outputs/train/libero_90_pi05_base_nomem_50k/checkpoints/last/pretrained_model"
RUN_DIR="$ROOT_DIR/outputs/train/loraft_multitask10_r256_50k"
...
if [ -d "$RUN_DIR/checkpoints/050000" ]; then
  echo "[mt10-r256] final checkpoint exists - skipping train."
elif [ -d "$RUN_DIR" ]; then
  ...
  echo "[mt10-r256] partial/stub output dir found -> moving aside to $ASIDE and RESTARTING FROM SCRATCH"
  mv "$RUN_DIR" "$ASIDE"
fi
if [ ! -d "$RUN_DIR/checkpoints/050000" ]; then
  lerobot-train \
    ...
    --job_name="loraft_multitask10_r256_50k"
fi
[ -d "$RUN_DIR/checkpoints/050000" ] || { echo "[mt10-r256] FATAL: 050000 missing"; exit 1; }
echo "E64 multitask-LoRA-10 r256/50k completed at $(date)"
