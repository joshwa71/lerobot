cat > train_smolvla_libero_10_lora.sh << 'EOF'
#!/bin/bash
#$ -S /bin/bash
#$ -l tmem=64G
#$ -l h_rt=72:00:00
...
#$ -R y
#$ -l tscratch=100G
#$ -N smolvla_libero_10_lora_train
...
OUTPUT_SCRATCH="$SCRATCH_DIR/outputs/train/smolvla_libero_10_lora"
FINAL_OUTPUT_DIR="/SAN/vision/jo71_vla_wd/lerobot_meta/outputs/train/smolvla_libero_10_lora"
...
lerobot-train \
  ...
  "(?:^|\\.)action_time_mlp_out$",
  "(?:^|\\.)connector(\\.\\w+)?$"]' \
  --job_name=smolvla_libero_10_lora \
  --wandb.enable=true 
