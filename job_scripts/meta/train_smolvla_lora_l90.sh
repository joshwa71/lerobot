cat > train_smolvla_libero_90_lora.sh << 'EOF'
#!/bin/bash
#$ -S /bin/bash
#$ -l tmem=64G
#$ -l h_rt=72:00:00
#$ -l gpu=true,gpu_type=(a100_80|a40|h100|rtx6000ada)
#$ -pe gpu 1
#$ -R y
#$ -l tscratch=100G
#$ -N smolvla_libero_90_lora_train
...
OUTPUT_SCRATCH="$SCRATCH_DIR/outputs/train/smolvla_libero_90_lora"
FINAL_OUTPUT_DIR="/SAN/vision/jo71_vla_wd/lerobot_meta/outputs/train/smolvla_libero_90_lora"
...
lerobot-train \
  ...
  --policy.repo_id=outputs/train/smolvla_lora \
  ...
  "(?:^|\\.)action_time_mlp_out$",
  "(?:^|\\.)connector(\\.\\w+)?$"]' \
  --job_name=smolvla_libero_90_lora \
  --wandb.enable=true 
