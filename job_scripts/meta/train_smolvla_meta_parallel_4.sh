cat > train_smolvla_meta_libero_10_parallel_4.sh << 'EOF'
#!/bin/bash
#$ -S /bin/bash
#$ -l tmem=64G
#$ -l h_rt=72:00:00
...
#$ -R y
#$ -l tscratch=200G
#$ -N smolvla_meta_libero_10_train_parallel_4
...
OUTPUT_SCRATCH="$SCRATCH_DIR/outputs/train/reptile_smolvla_libero_parallel_4"
FINAL_OUTPUT_DIR="/SAN/vision/jo71_vla_wd/lerobot_meta/outputs/train/reptile_smolvla_libero_parallel_4"
...
lerobot-meta-train \
  ...
  --policy.repo_id=outputs/train/reptile_smolvla_libero_parallel_4 \
  --lora.enable=true \
  --lora.r=8 \
  --num_workers=4 \
  --lora.alpha=16 \
  ...
  --job_name=reptile_smolvla_libero_parallel_4 \
