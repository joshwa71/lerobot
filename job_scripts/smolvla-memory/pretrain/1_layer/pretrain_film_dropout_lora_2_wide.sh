cat > smolvla_pretrain_train_film_dropout_lora_2_wide.sh << 'EOF'
#!/bin/bash
#$ -S /bin/bash
#$ -l tmem=64G
#$ -l h_rt=72:00:00
...
#$ -R y
#$ -l tscratch=200G
#$ -N smolvla_pretrain_train_film_dropout_lora_2_wide
...
OUTPUT_SCRATCH="$SCRATCH_DIR/outputs/train/libero_95_film_dropout_lora_2_wide"
# Final output target (used by trap for sync-back)
FINAL_OUTPUT_DIR="/SAN/vision/jo71_vla_wd/lerobot_memory/outputs/train/libero_95_film_dropout_lora_2_wide"
...
lerobot-train \
  ...
  --policy.repo_id=outputs/train/libero_95_film_dropout_lora_2_wide \
  ...
  --job_name=libero_95_film_dropout_lora_2_wide \
  ...
  --policy.memory_layer.mem_n_keys=768 \
