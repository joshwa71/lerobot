cat > train_pi0_mixed_libero_10_300k.sh << 'EOF'
#!/bin/bash
#$ -S /bin/bash
#$ -l tmem=64G
#$ -l h_rt=72:00:00
...
#$ -pe gpu 1
#$ -R y
#$ -N pi0_mixed_libero_10_300k_train
...
lerobot-train \
  ...
  --policy.repo_id=outputs/train/mixed_libero_10_pi0_300k \
  --dataset.repo_id=outputs/mixed_libero_10 \
  --output_dir=./outputs/train/mixed_libero_10_pi0_300k \
  --steps=300000 \
  --batch_size=16 \
  --eval_freq=0 \
  --save_freq=20000 \
  ...
  --job_name=mixed_libero_10_pi0_300k \
  --wandb.enable=true
